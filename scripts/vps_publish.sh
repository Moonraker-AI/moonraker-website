...
cd "$REPO"
# npm install, not npm ci: the lock is regenerated by whatever npm wrote it last
# (local dev runs a newer npm than this host), and a cross-version lock makes the
# strict `npm ci` fail (e.g. a peer dep present for one npm major, pruned by the
# other). `npm install` reconciles package.json against the lock and installs.
# git reset --hard above means any lock mutation here is transient.
log "npm install"
npm install --no-audit --no-fund --no-save --silent
log "astro build"
npx astro build
